...
class BasicService(object):
    ...
    def _process_incoming_packet(self, data, addr):
        ...
        counter = self.packet_queue.qsize()
        monitoring_report(counter)
    ...
    def monitoring_report(counter):
        cpu_update = 30 + counter*20;
        logger.info('%d : CPU-utilization ', cpu_update)
        # parse xml report
        tree = ET.parse('/home/mnc-boo/boo-netopeer/sf-desc-mon-rpt/service-function-description-monitor-report.xml')
        # get root node
        root = tree.getroot()
        for cpu in root.iter('CPU-utilization'):
            cpu.text = str(cpu_update);

        tree.write('/home/mnc-boo/boo-netopeer/sf-desc-mon-rpt/service-function-description-monitor-report.xml');

    def wait_processing (counter) :
        processing_delay_time = 0.1 + counter * 0.1;
        logger.info('%d : processing time ', processing_delay_time)
        # wait
        time.sleep (processing_delay_time)
